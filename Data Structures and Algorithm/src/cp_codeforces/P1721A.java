...
public class P1721A {
	...
	static MyFastReaderP1721A in = new MyFastReaderP1721A();
	...
	public static void main(String[] args) throws Exception {
		...
		while (test-- > 0) {
			...
			for(long[] arr: dp) Arrays.fill(arr,-1L);
			
			for(long[] arr: dp) System.out.println(Arrays.toString(arr));
			
			long ans=getAns(dp,0,0,sx,sy,n,m,d);
	...
	private static long getAns(long[][] dp, int i, int j, int sx, int sy,int n,int m, int d) {
		
		//System.out.println(": "+i+" "+j);
		
		if(i>=n || j>=m || i<0 || j<0 || (1L*Math.abs(sx-i)+ 1L*Math.abs(sy-j)) <=d) {
		...
		if(i==n-1 && j==m-1) return 0L;
		
		
		long a=getAns(dp,i+1,j,sx,sy,n,m,d);
		long b=getAns(dp,i-1,j,sx,sy,n,m,d);
		long c=getAns(dp,i,j+1,sx,sy,n,m,d);
		long c2=getAns(dp,i,j-1,sx,sy,n,m,d);
		
		
		dp[i][j]=1+ Math.min(a, Math.min(b, Math.min(c, c2)));
...
class MyFastReaderP1721A {
	BufferedReader br;
	StringTokenizer st;

	public MyFastReaderP1721A() {
